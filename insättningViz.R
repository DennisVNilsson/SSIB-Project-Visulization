...
OthersDf<- data.frame(Klass = c( "ArrayList", "Stack"),
...
# Creating data frame for LinkedList and SLL 
LinkedLists<- data.frame(Klass = c(  "LinkedList", "SLL"),
...
insättningUnLinked <- ggplot(OthersDf, aes(x=Klass, y=Mean, fill = Klass)) + 
...
# Add error bars
insättningUnLinked <- insättningUnLinked + labs(title="Medelvärde för insättninng av klasser med icke-länkad datastruktur", 
